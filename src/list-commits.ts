...
import { getLocalBranches, getCurrentBranch } from "./utils/git.js";
...
export async function listBranchesCLI(): Promise<void> {
  try {
    ...
    const toolBranches = branches.filter((branch) =>
      /^\d{4}-[a-f0-9]+$/.test(branch)
    );
    ...
    for (const branch of toolBranches) {
      ...
      const branchDisplay = isCurrent ? chalk.green.bold(branch) : branch;

      console.log(
        `${chalk.yellow(mark)}  ${chalk.green(seq)}  ${chalk.yellow(
          hash
        )}  ${branchDisplay}`
      );
    }
    ...
    console.log(
      chalk.blue(
        `${t("common.total")}: ${toolBranches.length} ${t("common.branches")}\n`
      )
    );
  } catch (error: unknown) {
 ...
 * @param file 檔案路徑
 */
export async function listAllCommitsCLI(
  file: string = "git-commits.log"
): Promise<void> {
  try {
    let commits;
    try {
    ...
    for (const commit of commits) {
      console.log(
        `${chalk.green(commit.seq)}  ${chalk.yellow(commit.hash)}  ${
          commit.message || ""
        }`
      );
    }
    ...
    console.log(
      chalk.blue(
        `${t("common.total")}: ${commits.length} ${t("common.commits")}\n`
      )
    );
  } catch (error: unknown) {
    const err = error as Error;
